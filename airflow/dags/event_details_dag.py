...
from event_details_producer import run_once 
...
with DAG(
    dag_id="event_stats_daily_dag",
    default_args=default_args,
    description='DAG to run event_stats producer once daily at midnight Egypt time',
    catchup=False,
    schedule='0 0 * * *',  # every day at midnight
    tags=['soccer.event.stats']
) as dag:

    task_run_event_stats = PythonOperator(
        task_id='run_event_stats_once',
        python_callable=run_once,
    )

    task_run_event_stats
