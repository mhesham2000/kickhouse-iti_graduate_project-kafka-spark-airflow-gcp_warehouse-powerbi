...
from venue_producer import run 
...
with DAG(
    dag_id="venue_proucer_daily_dag",
    default_args=default_args,
    description='DAG to run venue producer once daily at midnight Egypt time',
    catchup=False,
    schedule='0 0 * * *',  # every day at midnight
    tags=['soccer.venue']
) as dag:

    task_run_venue = PythonOperator(
        task_id='run_venue_producer_once',
        python_callable=run,
    )
