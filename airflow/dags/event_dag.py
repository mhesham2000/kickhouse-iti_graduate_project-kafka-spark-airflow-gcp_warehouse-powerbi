...
from event_producer import run 
...
with DAG(
    dag_id="event_proucer_daily_dag",
    default_args=default_args,
    description='DAG to run event producer once daily at midnight Egypt time',
    catchup=False,
    schedule='0 0 * * *',  # every day at midnight
    tags=['soccer.event']
) as dag:

    task_run_event = PythonOperator(
        task_id='run_event_producer_once',
        python_callable=run,
    )
